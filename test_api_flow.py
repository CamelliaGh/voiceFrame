...
def test_api_flow():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            update_response = requests.put(f"{base_url}/api/session/{session_token}", json=update_data)
            
            if update_response.status_code == 200:
                print("Session updated successfully")
                
                # Get the updated session
                updated_response = requests.get(f"{base_url}/api/session/{session_token}")
                if updated_response.status_code == 200:
                    ...
                    preview_response = requests.get(f"{base_url}/api/session/{session_token}/preview")
                    ...
                    print(f"Failed to get updated session: {updated_response.status_code}")
            else:
