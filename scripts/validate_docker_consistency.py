...
def parse_docker_compose(file_path: Path) -> Dict[str, Dict]:
    ...
    for match in matches:
        ...
        # Extract depends_on
        depends_matches = re.findall(r'(\w+):\s*\n\s*condition:\s*(\w+)', service_content)
...
def validate_docker_consistency():
    ...
    if dockerfile['base_image'] and dockerfile_test['base_image']:
        if dockerfile['base_image'] != dockerfile_test['base_image']:
            issues.append(f"Base image mismatch: {dockerfile['base_image']} vs {dockerfile_test['base_image']}")
        ...
        if service_info['build'] == '.':
            ...
            if 'celery' in service_name.lower():
                ...
                if api_service.get('environment') != service_info.get('environment'):
                    issues.append(f"Environment mismatch between api and {service_name}")
    ...
    if requirements_file.exists():
        ...
        required_python_packages = {
            'psutil', 'celery', 'boto3', 'pillow', 'librosa', 'soundfile',
            'pydub', 'matplotlib', 'scipy', 'reportlab', 'qrcode', 'fastapi',
            'uvicorn', 'sqlalchemy', 'redis', 'stripe', 'sendgrid'
        }
        ...
        if missing_python_packages:
            issues.append(f"Missing required Python packages: {missing_python_packages}")
