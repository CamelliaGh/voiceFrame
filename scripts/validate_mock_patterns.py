            ...
            if isinstance(node, ast.Call):
                ...
                if (
                    ...
                    if len(node.args) >= 2:
                        ...
                        if (
                            isinstance(class_name, ast.Name)
                            and isinstance(attribute_name, ast.Constant)
                            and class_name.id in ["PDFGenerator", "VisualPDFGenerator"]
                            and attribute_name.value
                            in ["file_uploader", "template_service", "image_processor"]
                        ):
                            issues.append(
                                f"Potentially incorrect @patch.object for {class_name.id}.{attribute_name.value} - consider mocking instance attributes instead"
                            )
