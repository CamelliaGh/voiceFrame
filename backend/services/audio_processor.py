...
try:
    ...
    logging.warning("scipy not available - envelope peak detection will use alternative method")
...
class AudioProcessor:
    ...
    def _process_audio_in_chunks(self, file_path: str, sample_rate: int) -> Dict[str, any]:
        ...
        try:
            ...
            for chunk_idx in range(num_chunks):
                ...
                logger.debug(f"Processing chunk {chunk_idx + 1}/{num_chunks}: {start_time:.1f}s - {end_time:.1f}s")
            ...
            logger.info(f"Chunk processing completed: {aggregated_metrics['chunks_processed']} chunks")
    ...
    def _process_audio_chunk(self, audio_data: np.ndarray, sample_rate: int, chunk_idx: int) -> Dict[str, any]:
        ...
        try:
            ...
            if len(audio_data) > 1024:  # Only if chunk is large enough
                try:
                    ...
                    chunk_metrics['spectral_centroid'] = float(np.mean(spectral_centroid))
                    
                    zcr = librosa.feature.zero_crossing_rate(audio_data)[0]
                    chunk_metrics['zero_crossing_rate'] = float(np.mean(zcr))
                except Exception as e:
                    logger.warning(f"Spectral analysis failed for chunk {chunk_idx}: {str(e)}")
    ...
    def _aggregate_chunk_results(self, aggregated_metrics: Dict[str, any]) -> Dict[str, any]:
        ...
        try:
            ...
            final_metrics = {
                ...
                'dynamic_range': 20 * np.log10(total_peak / (total_rms + 1e-10)) if total_rms > 0 else 0,
                'avg_spectral_centroid': total_spectral_centroid if valid_chunks > 0 else 0,
    ...
    def validate_audio_file(self, file_path: str) -> Dict[str, any]:
        ...
        try:
            ...
            try:
                y, sr = librosa.load(file_path, sr=None, duration=1.0)  # Load only first second for validation
                ...
                if sr < 8000:  # Minimum sample rate
                    raise AudioValidationError(f"Sample rate too low: {sr} Hz (minimum: 8000 Hz)")
                
                if sr > 192000:  # Maximum sample rate
                    raise AudioValidationError(f"Sample rate too high: {sr} Hz (maximum: 192000 Hz)")
    ...
    def analyze_audio_quality(self, file_path: str, audio_data: np.ndarray, sample_rate: int) -> Dict[str, any]:
        """
        ...
        try:
            ...
            # 3. Spectral analysis
            quality_metrics.update(self._analyze_spectral_features(audio_data, sample_rate))
            
            # 4. Voice activity detection for speech quality
            quality_metrics.update(self._analyze_voice_activity(audio_data, sample_rate))
            
            # 5. Audio quality scoring
            quality_metrics['quality_score'] = self._calculate_quality_score(quality_metrics)
            
            # 6. Noise level analysis
            quality_metrics['noise_level'] = self._analyze_noise_level(audio_data, sample_rate)
            
            logger.info(f"Audio quality analysis completed: {len(quality_metrics)} metrics")
    ...
    def _analyze_bitrate(self, file_path: str) -> Dict[str, any]:
        ...
        try:
            ...
            # Calculate bitrate
            bitrate = audio_segment.frame_rate * audio_segment.frame_width * audio_segment.channels * 8
    ...
    def _analyze_spectral_features(self, audio_data: np.ndarray, sample_rate: int) -> Dict[str, any]:
        ...
        try:
            ...
            return {
                ...
                'mfcc_means': [float(x) for x in mfcc_means[:5]]  # First 5 MFCC coefficients
            }
    ...
    def _analyze_voice_activity(self, audio_data: np.ndarray, sample_rate: int) -> Dict[str, any]:
    ...
    def _analyze_noise_level(self, audio_data: np.ndarray, sample_rate: int) -> Dict[str, any]:
    ...
    async def process_audio_file(self, audio_s3_key: str, session_token: str) -> dict:
        ...
        try:
            logger.info(f"Processing audio file: {audio_s3_key} for session: {session_token}")
            ...
            if self._should_use_chunk_processing(duration, file_size):
                logger.info(f"Using chunk-based processing for large file: {duration:.1f}s, {file_size/1024/1024:.1f}MB")
            ...
            y_trimmed, trimming_info = self.advanced_silence_trimming(y, sr, trim_method='adaptive')
            trimming_time = time.time() - trimming_start
            self.performance_stats['processing_times']['advanced_trimming'] = trimming_time
            ...
            peak_results = self.detect_peaks(y_trimmed, sr, peak_types=['onset', 'spectral', 'envelope'])
            peak_detection_time = time.time() - peak_detection_start
            self.performance_stats['processing_times']['peak_detection'] = peak_detection_time
            ...
            logger.info(f"Audio processing completed successfully for session: {session_token}")
            logger.info(f"Total processing time: {total_time:.2f}s, Memory used: {memory_used:.1f}MB")
            ...
            if audio_path and os.path.exists(audio_path) and not audio_s3_key.startswith('temp_'):
                try:
                    os.unlink(audio_path)
                    logger.debug(f"Cleaned up temporary file: {audio_path}")
                except Exception as e:
                    logger.warning(f"Failed to cleanup temporary file {audio_path}: {str(e)}")
        ...
        try:
            ...
            if s3_key.startswith('temp_'):
                ...
                if temp_path and os.path.exists(temp_path):
                    ...
                    logger.warning(f"Local temp file not found, will download from S3: {s3_key}")
    ...
    def _generate_waveform_image(self, audio_data: np.ndarray, sample_rate: int, 
        ...
        try:
            start_time = time.time()
            logger.debug(f"Generating waveform: {len(audio_data)} samples, {sample_rate}Hz")
            ...
            if len(audio_data) > max_samples:
                ...
                logger.debug(f"Downsampled audio data from {len(audio_data) * step} to {len(audio_data)} samples")
            ...
            logger.debug(f"Waveform generated: {len(buffer.getvalue())} bytes in {processing_time:.2f}s")
            
            # Store performance stats
            self.performance_stats['processing_times']['waveform_generation'] = processing_time
    ...
    async def _upload_waveform(self, waveform_buffer: BytesIO, session_token: str) -> str:
        ...
        try:
            ...
            if temp_file and os.path.exists(temp_file.name):
                try:
                    os.unlink(temp_file.name)
                    logger.debug(f"Cleaned up temporary waveform file: {temp_file.name}")
                except Exception as e:
                    logger.warning(f"Failed to cleanup temporary file {temp_file.name}: {str(e)}")
    
    def advanced_silence_trimming(self, audio_data: np.ndarray, sample_rate: int, 
                                 trim_method: str = 'dynamic') -> Tuple[np.ndarray, Dict[str, any]]:
        """
        ...
        """
        try:
            logger.info(f"Applying advanced silence trimming using method: {trim_method}")
    ...
    def _dynamic_silence_trimming(self, audio_data: np.ndarray, sample_rate: int) -> Tuple[np.ndarray, Dict[str, any]]:
        ...
        try:
            ...
            if rms_std > 0:
                ...
                threshold_db = max(-40, min(-10, threshold_db))  # Clamp between -40 and -10 dB
            else:
            ...
            trimmed_audio, trim_indices = librosa.effects.trim(audio_data, top_db=abs(threshold_db))
            ...
            trimming_info = {
                ...
                'silence_percentage': float((silence_removed / original_duration) * 100) if original_duration > 0 else 0,
                'trim_indices': trim_indices.tolist()
            }
            
            logger.info(f"Dynamic trimming completed: {silence_removed:.2f}s removed ({trimming_info['silence_percentage']:.1f}%)")
    ...
    def _split_based_trimming(self, audio_data: np.ndarray, sample_rate: int) -> Tuple[np.ndarray, Dict[str, any]]:
        ...
        try:
            ...
            if len(intervals) == 0:
                logger.warning("No non-silent intervals found, returning original audio")
                return audio_data, {'method': 'split', 'intervals': 0, 'error': 'no_intervals'}
            ...
            trimming_info = {
                ...
                'silence_removed': float(silence_removed),
                'silence_percentage': float((silence_removed / original_duration) * 100) if original_duration > 0 else 0,
                'interval_details': [{'start': int(start), 'end': int(end), 'duration': float((end-start)/sample_rate)} 
                                   for start, end in intervals]
            }
            
            logger.info(f"Split-based trimming completed: {len(intervals)} intervals, {silence_removed:.2f}s removed")
            return trimmed_audio, trimming_info
    ...
    def _segment_based_trimming(self, audio_data: np.ndarray, sample_rate: int) -> Tuple[np.ndarray, Dict[str, any]]:
        ...
        try:
            ...
            for i, segment in enumerate(segments):
                try:
                    ...
                    if rms_mean > 0.01:  # If segment has significant content
                        threshold_db = -15 - (rms_mean * 20)  # Adaptive based on segment energy
                        threshold_db = max(-35, min(-5, threshold_db))
                    else:
                        threshold_db = -25  # Lower threshold for quiet segments
                    
                    trimmed_segment, trim_indices = librosa.effects.trim(segment, top_db=abs(threshold_db))
            ...
            if not trimmed_segments:
                logger.warning("All segments were empty after trimming, returning original audio")
                return audio_data, {'method': 'segment', 'segments': len(segments), 'error': 'all_segments_empty'}
            ...
            logger.info(f"Segment-based trimming completed: {len(trimmed_segments)}/{len(segments)} segments kept")
            return trimmed_audio, trimming_info
    ...
    def _adaptive_trimming(self, audio_data: np.ndarray, sample_rate: int) -> Tuple[np.ndarray, Dict[str, any]]:
        """Adaptive trimming that combines multiple strategies"""
        try:
            # First, try split-based trimming
            try:
                trimmed_audio, split_info = self._split_based_trimming(audio_data, sample_rate)
                ...
                logger.warning(f"Split-based trimming failed in adaptive mode: {str(e)}")
            
            # Fallback to dynamic trimming
            try:
                trimmed_audio, dynamic_info = self._dynamic_silence_trimming(audio_data, sample_rate)
    ...
    def detect_peaks(self, audio_data: np.ndarray, sample_rate: int, 
                    peak_types: List[str] = ['onset', 'spectral', 'envelope']) -> Dict[str, any]:
        """
        ...
        try:
            ...
            if 'onset' in peak_types:
                peak_results['onset_peaks'] = self._detect_onset_peaks(audio_data, sample_rate)
            
            # Spectral peak detection
            if 'spectral' in peak_types:
                peak_results['spectral_peaks'] = self._detect_spectral_peaks(audio_data, sample_rate)
            
            # Envelope peak detection
            if 'envelope' in peak_types:
                peak_results['envelope_peaks'] = self._detect_envelope_peaks(audio_data, sample_rate)
            
            # Calculate overall peak statistics
            peak_results['peak_statistics'] = self._calculate_peak_statistics(peak_results)
    ...
    def _detect_onset_peaks(self, audio_data: np.ndarray, sample_rate: int) -> Dict[str, any]:
        ...
        try:
            ...
            return {
                ...
                'onset_rate': float(len(onset_frames) / (len(audio_data) / sample_rate)) if len(audio_data) > 0 else 0
            }
    ...
    def _detect_spectral_peaks(self, audio_data: np.ndarray, sample_rate: int) -> Dict[str, any]:
        ...
        try:
            ...
            for i in range(mel_spec_db.shape[0]):  # For each mel bin
                ...
                peaks, _ = librosa.util.peak_pick(mel_data, pre_max=3, post_max=3, pre_avg=3, post_avg=5, delta=0.2, wait=10)
                if len(peaks) > 0:
                    for peak in peaks:
                        mel_peaks.append({
                            'mel_bin': int(i),
            ...
            for i in range(chroma.shape[0]):  # For each chroma bin
                ...
                peaks, _ = librosa.util.peak_pick(chroma_data, pre_max=2, post_max=2, pre_avg=2, post_avg=3, delta=0.1, wait=5)
                if len(peaks) > 0:
                    for peak in peaks:
                        chroma_peaks.append({
                            'chroma_bin': int(i),
    ...
    def _detect_envelope_peaks(self, audio_data: np.ndarray, sample_rate: int) -> Dict[str, any]:
        ...
        try:
            ...
            return {
                ...
                'peak_rate': float(len(peaks) / (len(audio_data) / sample_rate)) if len(audio_data) > 0 else 0,
    ...
    def _calculate_peak_statistics(self, peak_results: Dict[str, any]) -> Dict[str, any]:
        ...
        try:
            ...
            for key, value in peak_results.items():
                if key.endswith('_peaks') and isinstance(value, dict):
                    ...
                    peak_count = value.get(f'{peak_type}_count', value.get('peak_count', 0))
    ...
    def extract_audio_features(self, audio_data: np.ndarray, sample_rate: int) -> dict:
        ...
        try:
            logger.debug(f"Extracting audio features: {len(audio_data)} samples, {sample_rate}Hz")
