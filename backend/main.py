...
from .models import Base, SessionModel, Order, EmailSubscriber
from .schemas import (
    SessionResponse, SessionUpdate, UploadResponse, ProcessingStatus,
    PaymentIntentRequest, PaymentIntentResponse, CompleteOrderRequest,
    DownloadResponse
)
...
async def get_processing_status(token: str, db: Session = Depends(get_db)):
    ...
    return ProcessingStatus(
        ...
        preview_ready=session.photo_s3_key is not None and session.waveform_s3_key is not None
    )
...
async def create_payment_intent(
    ...
    try:
        ...
        order = Order(
            ...
            download_expires_at=datetime.utcnow() + timedelta(days=7)  # 7 days for all users
        )
        db.add(order)
        db.commit()
...
async def complete_order(
    ...
    try:
        ...
        subscriber = db.query(EmailSubscriber).filter(EmailSubscriber.email == order.email).first()
        if not subscriber:
        ...
        raise HTTPException(status_code=500, detail=f"Order completion failed: {str(e)}")
...
async def listen_to_audio(identifier: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if order and order.permanent_audio_s3_key:
            ...
            audio_url = permanent_audio_service.get_permanent_audio_url(order, expiration=3600)
            ...
            if session and session.audio_s3_key:
                ...
                audio_url = file_uploader.generate_presigned_url(session.audio_s3_key, expiration=3600)
...
async def update_session_template(token: str, template_id: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        return {"message": f"Template updated to {template_id}", "template_id": template_id}
