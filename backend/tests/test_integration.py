...
from .conftest import TestOrder as Order
...
class TestFileMigrationIntegration:
    ...
    def test_complete_payment_flow_with_migration(
        self,
        client,
        sample_session,
        mock_s3_client,
        ...
        payment_data = {"email": "test@example.com"}

        response = client.post(
            f"/api/session/{sample_session.session_token}/payment", json=payment_data
        )
        assert response.status_code == 200
        order_id = response.json()["order_id"]

        # Complete order with payment
        complete_data = {"payment_intent_id": "pi_test"}

        with patch(
            "backend.main.pdf_generator.generate_final_pdf"
        ) as mock_generate_pdf:
            mock_generate_pdf.return_value = "https://example.com/test.pdf"

            response = client.post(
                f"/api/orders/{order_id}/complete", json=complete_data
            )
    ...
    def test_migration_failure_handling(
        self, client, sample_session, mock_s3_client, mock_stripe_service
    ):
        """Test handling of migration failures"""
        # Mock storage manager to fail
        with patch("backend.main.storage_manager") as mock_storage:
            mock_storage.migrate_all_session_files.side_effect = Exception(
                "Migration failed"
            )
            ...
            response = client.post(
                f"/api/session/{sample_session.session_token}/payment",
                json=payment_data,
            )
            order_id = response.json()["order_id"]
    ...
    def test_session_update_validation(self, client, sample_session):
        ...
        response = client.put(
            f"/api/session/{sample_session.session_token}", json=valid_data
        )
        assert response.status_code == 200
        ...
        response = client.put(
            f"/api/session/{sample_session.session_token}", json=none_data
        )
        assert response.status_code == 200
        ...
        response = client.put(
            f"/api/session/{sample_session.session_token}", json=invalid_data
        )
        assert response.status_code == 422  # Validation error

    def test_qr_code_expiration_differences(
        self, client, sample_session, sample_order, mock_s3_client
    ):
        """Test that QR codes have different expiration times for paid vs unpaid"""
        with patch(
            "backend.services.pdf_generator.FileUploader"
        ) as mock_file_uploader_class:
    ...
    def test_cleanup_tasks_manual_only(self, client, sample_session):
        ...
        assert (
            not hasattr(celery_app.conf, "beat_schedule")
            or not celery_app.conf.beat_schedule
        )
