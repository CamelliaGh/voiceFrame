...
class TestStorageManagerMigration:
    ...
    async def test_migrate_all_session_files_success(
        ...
        result = await storage_manager.migrate_all_session_files(
            mock_session.session_token, str(mock_order.id)
        )
        ...
        # Verify file paths
        assert (
            result["permanent_photo_s3_key"] == f"permanent/photos/{mock_order.id}.jpg"
        )
        assert (
            result["permanent_audio_s3_key"] == f"permanent/audio/{mock_order.id}.mp3"
        )
        assert (
            result["permanent_waveform_s3_key"]
            == f"permanent/waveforms/{mock_order.id}.png"
        )
    ...
    def test_get_audio_content_type(self, storage_manager):
        ...
        assert (
            storage_manager._get_audio_content_type("unknown") == "audio/mpeg"
        )  # default
...
class TestFileMigrationIntegration:
    ...
    def test_complete_order_with_migration_success(
        ...
        mock_get_session,
        mock_verify_payment,
        mock_get_session_by_order,
        mock_verify_migration,
        mock_migrate_files,
        client,
    ):
