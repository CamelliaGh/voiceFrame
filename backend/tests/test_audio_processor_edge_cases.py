...
from backend.services.audio_processor import AudioProcessor, AudioProcessingError, AudioFormatError, AudioValidationError
...
class TestAudioProcessorEdgeCases:
    ...
    def test_single_sample_audio(self, audio_processor, mock_file_uploader):
        ...
        # Test metadata extraction
        result = audio_processor.extract_audio_metadata_from_data(audio_data, sample_rate)
    ...
    def test_oversized_duration_audio(self, audio_processor, mock_file_uploader):
        ...
        # Test validation
        with pytest.raises(AudioValidationError, match="Audio duration exceeds maximum"):
    ...
    def test_invalid_trimming_method(self, audio_processor, mock_file_uploader):
        ...
        with pytest.raises(AudioProcessingError, match="Invalid trimming method"):
            audio_processor.advanced_silence_trimming(audio_data, sample_rate, method='invalid')
    ...
    def test_recovery_from_processing_error(self, audio_processor, mock_file_uploader):
        ...
        with patch.object(audio_processor, 'analyze_audio_quality', side_effect=AudioProcessingError("Processing error")):
            # Test that error is properly handled
            with pytest.raises(AudioProcessingError, match="Processing error"):
                audio_processor.analyze_audio_quality("test.wav", audio_data, sample_rate)
    ...
    def test_memory_cleanup_after_error(self, audio_processor, mock_file_uploader):
        ...
        assert memory_increase < 50.0, f"Memory increase too high after error: {memory_increase}MB"
    ...
    def test_memory_cleanup_after_success(self, audio_processor, mock_file_uploader):
        ...
        # Verify memory increase is reasonable (should be < 50MB)
        assert memory_increase < 50.0, f"Memory increase too high after success: {memory_increase}MB"
    ...
    async def test_concurrent_error_handling(self, audio_processor, mock_file_uploader):
        ...
        for i in range(3):
            ...
            task = audio_processor.process_audio_file(f"temp_audio/invalid{i}.mp3", f"test-session-{i}")
            tasks.append(task)
    ...
    @pytest.mark.asyncio
    async def test_mixed_success_failure_concurrent(self, audio_processor, mock_file_uploader):
        ...
        try:
            ...
            # Valid task
            audio_processor._download_audio_file = AsyncMock(return_value=valid_file.name)
            ...
            with patch('backend.services.audio_processor.settings') as mock_settings:
                ...
                # Valid task
                task1 = audio_processor.process_audio_file("temp_audio/valid.mp3", "test-session-1")
                tasks.append(task1)
                ...
                task2 = audio_processor.process_audio_file("temp_audio/invalid.mp3", "test-session-2")
                tasks.append(task2)
