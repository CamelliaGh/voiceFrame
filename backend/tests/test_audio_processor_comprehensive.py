...
class TestAudioProcessorComprehensive:
    ...
    def sample_audio_data(self):
        ...
        audio_data[middle_start:middle_end] = 0.5 * np.sin(
            2 * np.pi * 440 * t[middle_start:middle_end]
        )
        audio_data[middle_start:middle_end] += 0.3 * np.sin(
            2 * np.pi * 880 * t[middle_start:middle_end]
        )

        # Add some random peaks for peak detection testing
        peak_indices = np.random.choice(
            np.arange(middle_start, middle_end, 1000), size=5, replace=False
        )
        for idx in peak_indices:
            audio_data[idx : idx + 100] = 0.8 * np.sin(
                2 * np.pi * 1000 * t[idx : idx + 100]
            )
    ...
    def test_extract_audio_metadata_corrupted_file(self, audio_processor):
        ...
        try:
            with pytest.raises(
                AudioProcessingError, match="Metadata extraction failed"
            ):
    ...
    def test_analyze_audio_quality_comprehensive(
        self, audio_processor, sample_audio_data
    ):
    ...
    def test_analyze_bitrate_with_pydub(self, audio_processor):
        ...
        with patch("backend.services.audio_processor.PYDUB_AVAILABLE", True):
            with patch(
                "backend.services.audio_processor.AudioSegment"
                ...
                assert "channels" in result
                assert result["encoding_quality"] in [
                    "high",
                    "medium",
                    "standard",
                    "low",
    ...
    def test_analyze_spectral_features(self, audio_processor, sample_audio_data):
        ...
        spectral_features = audio_processor._analyze_spectral_features(
    ...
    def test_analyze_voice_activity(self, audio_processor, sample_audio_data):
        ...
        voice_activity = audio_processor._analyze_voice_activity(
            audio_data, sample_rate
        )
    ...
    def test_analyze_noise_level(self, audio_processor, sample_audio_data):
        ...
        expected_features = ["noise_floor", "noise_ratio", "noise_level", "noise_score"]
    ...
    def test_advanced_silence_trimming_all_methods(
        self, audio_processor, sample_audio_data
    ):
        ...
        for method in methods:
            ...
            assert (
                trimming_info["trimmed_duration"] <= trimming_info["original_duration"]
            )

    def test_advanced_silence_trimming_invalid_method(
        self, audio_processor, sample_audio_data
    ):
    ...
    def test_chunk_processing_large_file(
        self, audio_processor, temp_large_audio_file, large_audio_data
    ):
        ...
        chunk_metrics = audio_processor._process_audio_in_chunks(
            temp_large_audio_file, sample_rate
        )
    ...
    def test_chunk_processing_decision(self, audio_processor):
        ...
        should_chunk_small = audio_processor._should_use_chunk_processing(
            60.0, 10 * 1024 * 1024
        )  # 1 min, 10MB
        assert should_chunk_small is False

        # Test with long file (should use chunks)
        should_chunk_long = audio_processor._should_use_chunk_processing(
            180.0, 10 * 1024 * 1024
        )  # 3 min, 10MB
        assert should_chunk_long is True

        # Test with large file (should use chunks)
        should_chunk_large = audio_processor._should_use_chunk_processing(
            60.0, 60 * 1024 * 1024
        )  # 1 min, 60MB
    ...
    def test_generate_waveform_image(self, audio_processor, sample_audio_data):
        ...
        waveform_buffer = audio_processor._generate_waveform_image(
            audio_data, sample_rate
        )
    ...
    def test_generate_waveform_image_large_data(
        self, audio_processor, large_audio_data
    ):
    ...
    def test_quality_analysis_error_handling(self, audio_processor):
        ...
        result = audio_processor.analyze_audio_quality(
            "test.mp3", corrupted_audio, 44100
        )
    ...
    @pytest.mark.asyncio
    async def test_process_audio_file_integration(
        self, audio_processor, temp_audio_file, mock_file_uploader
    ):
    ...
    @pytest.mark.asyncio
    async def test_process_audio_file_chunk_processing(
        self, audio_processor, temp_large_audio_file, mock_file_uploader
    ):
        """Test audio processing with chunk processing"""
        # Mock the download and upload methods
        audio_processor._download_audio_file = AsyncMock(
            return_value=temp_large_audio_file
        )
        audio_processor._upload_waveform = AsyncMock(return_value="waveforms/test.png")
    ...
    @pytest.mark.asyncio
    async def test_process_audio_file_validation_error(
        self, audio_processor, temp_corrupted_audio_file, mock_file_uploader
    ):
        """Test audio processing with validation error"""
        # Mock the download method
        audio_processor._download_audio_file = AsyncMock(
            return_value=temp_corrupted_audio_file
        )
    ...
    def test_performance_stats_tracking(self, audio_processor, sample_audio_data):
        ...
        assert (
            "waveform_generation"
            in audio_processor.performance_stats["processing_times"]
        )

        processing_time = audio_processor.performance_stats["processing_times"][
            "waveform_generation"
        ]
