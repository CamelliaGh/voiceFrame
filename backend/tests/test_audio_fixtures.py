...
class AudioTestFixtures:
    ...
    def create_audio_with_silence(
        duration: float,
        sample_rate: int = 44100,
        silence_start: float = 0.2,
        silence_end: float = 0.4,
    ) -> np.ndarray:
    ...
    @staticmethod
    def create_audio_with_peaks(
        duration: float, sample_rate: int = 44100, num_peaks: int = 5
    ) -> np.ndarray:
    ...
    def create_temp_audio_file(
        audio_data: np.ndarray, sample_rate: int, suffix: str = ".wav"
    ) -> str:
...
def mock_file_operations():
    ...
    with patch("os.path.exists") as mock_exists, patch(
        "os.path.getsize"
    ) as mock_getsize, patch("os.unlink") as mock_unlink:
        mock_exists.return_value = True
        mock_getsize.return_value = 1024 * 1024  # 1MB
        mock_unlink.return_value = None

        yield {"exists": mock_exists, "getsize": mock_getsize, "unlink": mock_unlink}
