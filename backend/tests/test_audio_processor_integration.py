...
class TestAudioProcessorIntegration:
    ...
    async def test_complete_audio_processing_pipeline(
        self, audio_processor, sample_audio_file, mock_file_uploader
    ):
        ...
        with patch("backend.services.audio_processor.settings") as mock_settings:
            ...
            result = await audio_processor.process_audio_file(
                "temp_audio/test.mp3", "test-session"
            )
            ...
            audio_processor._download_audio_file.assert_called_once_with(
                "temp_audio/test.mp3"
            )
            audio_processor._upload_waveform.assert_called_once()

    @pytest.mark.asyncio
    async def test_complete_pipeline_with_chunk_processing(
        ...
        with patch("backend.services.audio_processor.settings") as mock_settings:
            ...
            result = await audio_processor.process_audio_file(
                "temp_audio/large.mp3", "test-session"
            )
    ...
    @pytest.mark.asyncio
    async def test_pipeline_with_s3_download_failure(
        ...
        audio_processor._download_audio_file = AsyncMock(
            side_effect=AudioProcessingError("Download failed")
        )

        # Test that the error is properly propagated
        with pytest.raises(Exception):  # Should raise HTTPException from FastAPI
            await audio_processor.process_audio_file(
                "temp_audio/test.mp3", "test-session"
            )

    @pytest.mark.asyncio
    async def test_pipeline_with_waveform_upload_failure(
        self, audio_processor, sample_audio_file, mock_file_uploader
    ):
        ...
        audio_processor._download_audio_file = AsyncMock(return_value=sample_audio_file)
        audio_processor._upload_waveform = AsyncMock(
            side_effect=AudioProcessingError("Upload failed")
        )
    ...
    @pytest.mark.asyncio
    async def test_s3_download_integration(
        self, audio_processor, sample_audio_file, mock_file_uploader
    ):
        ...
        with patch("tempfile.NamedTemporaryFile") as mock_temp_file:
            ...
            with patch("backend.services.audio_processor.settings") as mock_settings:
                ...
                # Test S3 download
                result_path = await audio_processor._download_audio_file(
                    "temp_audio/test.mp3"
                )
    ...
    async def test_s3_upload_integration(self, audio_processor, mock_file_uploader):
        ...
        with patch("tempfile.NamedTemporaryFile") as mock_temp_file:
            ...
            with patch("builtins.open", MagicMock()) as mock_open:
                mock_open.return_value.__enter__.return_value = BytesIO(
                    b"fake_png_data"
                )

                # Mock settings
                with patch(
                    "backend.services.audio_processor.settings"
                ) as mock_settings:
                    mock_settings.s3_bucket = "test-bucket"

                    # Test S3 upload
                    result_key = await audio_processor._upload_waveform(
                        waveform_buffer, "test-session"
                    )
    ...
    def test_quality_analysis_integration(self, audio_processor, sample_audio_file):
        ...
        quality_metrics = audio_processor.analyze_audio_quality(
            sample_audio_file, audio_data, sample_rate
        )
    ...
    async def test_pipeline_error_handling_invalid_file(
        ...
        try:
            ...
            with pytest.raises(Exception):  # Should raise HTTPException from FastAPI
                await audio_processor.process_audio_file(
                    "temp_audio/invalid.mp3", "test-session"
                )
    ...
    @pytest.mark.asyncio
    async def test_pipeline_error_handling_oversized_file(
        self, audio_processor, mock_file_uploader
    ):
        ...
        try:
            ...
            audio_processor._download_audio_file = AsyncMock(
                return_value=oversized_file.name
            )

            # Test that size validation error is properly handled
            with pytest.raises(Exception):  # Should raise HTTPException from FastAPI
                await audio_processor.process_audio_file(
                    "temp_audio/oversized.mp3", "test-session"
                )
    ...
    @pytest.mark.asyncio
    async def test_pipeline_error_handling_unsupported_format(
        self, audio_processor, mock_file_uploader
    ):
        ...
        try:
            ...
            audio_processor._download_audio_file = AsyncMock(
                return_value=unsupported_file.name
            )

            # Test that format error is properly handled
            with pytest.raises(Exception):  # Should raise HTTPException from FastAPI
                await audio_processor.process_audio_file(
                    "temp_audio/unsupported.xyz", "test-session"
                )
    ...
    @pytest.mark.asyncio
    async def test_pipeline_performance_tracking(
        self, audio_processor, sample_audio_file, mock_file_uploader
    ):
    ...
    @pytest.mark.asyncio
    async def test_pipeline_memory_usage_tracking(
        self, audio_processor, sample_audio_file, mock_file_uploader
    ):
    ...
    @pytest.mark.asyncio
    async def test_pipeline_cleanup_after_success(
        self, audio_processor, sample_audio_file, mock_file_uploader
    ):
    ...
    @pytest.mark.asyncio
    async def test_pipeline_cleanup_after_error(
        self, audio_processor, mock_file_uploader
    ):
